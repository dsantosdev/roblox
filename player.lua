...
local function salvarPos()
    if writefile then
        ...
        if not __ok then warn("salvarPos erro:", __e) end
...
UIS.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 then
        if dragging then salvarPos() end
        dragging = false
    end
end)
...
local function renderPlayers()
    ...
    for i, p in ipairs(lista) do
        ...
        local btnDefs = {
            { icon = "👣", mode = "follow", bg = Color3.fromRGB(15, 35, 55),  stroke = Color3.fromRGB(20, 70, 130),   tip = "Seguir" },
            { icon = "🪑", mode = "head",   bg = Color3.fromRGB(40, 25, 55),  stroke = Color3.fromRGB(80, 40, 120),   tip = "Cabeça" },
            { icon = "👻", mode = "inside", bg = Color3.fromRGB(15, 40, 40),  stroke = Color3.fromRGB(20, 100, 100),  tip = "Dentro" },
            { icon = "🔵", mode = "orbit",  bg = Color3.fromRGB(35, 25, 10),  stroke = Color3.fromRGB(120, 80, 20),   tip = "Orbitar"},
        }
        ...
        local modeColors = {
            ...
            head   = { row = Color3.fromRGB(25, 15, 35),     bar = Color3.fromRGB(180, 100, 255),  text = Color3.fromRGB(200, 150, 255),  status = "🪑 NA CABEÇA DE " },
...
closeBtn.MouseButton1Click:Connect(function()
    ...
    pararFollow()
    gui.Enabled = false
    if _G.Hub then pcall(function() _G.Hub.desligar("Follow Player") end) end
end)
...
local function onToggle(ativo)
    if not ativo then pararFollow() end
...
if _G.Hub then
    ...
    table.insert(_G.HubFila, { nome = "Follow Player", toggleFn = onToggle, categoria = CATEGORIA, jaAtivo = true })
end
